...
def evaluate_ears(imgdir,mode_path='logs/ear.h5',isDetail=True):
    img_width, img_height = 100, 100
    files = os.listdir(imgdir)
    n= len(files)
    i = 0
    model = load_model(mode_path)
    labels = ['Non-corn ear', '1 corn ear', '2 corn ears', '3 corn ears']
    ...
    for file in files:
        ...
        if isDetail:
            print('*'*40)
    print('Non-corn ear: {}, 1 corn ear: {}, 2 corn ears: {}, 3 corn ears: {}'.format(nonear,oneear,twoears,threeears))
    print('ears:',oneear+twoears*2+threeears*3)

if __name__=='__main__':
    evaluate_ears(r'data\test1',isDetail=False)